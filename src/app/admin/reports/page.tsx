...
export default function ReportsPage() {
  ...
  const processedData = Array.isArray(trendData) ? (() => {
    ...
    return trendData.reduce((acc: any[], item: TrendData) => {
      const existing = acc.find(d => d.date === item.date);
      if (existing) {
        const totalCount = existing.count + (item.testCount || 1);
        existing.avgDownload = (existing.avgDownload * existing.count + item.avgDownload * (item.testCount || 1)) / totalCount;
        existing.avgUpload = (existing.avgUpload * existing.count + item.avgUpload * (item.testCount || 1)) / totalCount;
        existing.avgPing = (existing.avgPing * existing.count + item.avgPing * (item.testCount || 1)) / totalCount;
  ...
  const yPingScale = (value: number) => chartHeight - padding.bottom - (value / maxPingScale) * (chartHeight - padding.top - padding.bottom);
  // Event handlers
      ...
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="mt-2 text-gray-600">Monitor and analyze internet speed performance across all office locations</p>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-200">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">Customize your view by selecting specific criteria</p>
                </div>
                <div className="flex items-center space-x-4">
                  <button
                    onClick={() => setFilters({
            ...
            {(filters.unit || filters.subunit || filters.section || filters.isp || filters.timeOfDay) && (
                ...
                <div className="flex items-center space-x-2 flex-wrap">
                  ...
                  {filters.unit && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      Unit: {filters.unit}
                      <button
                        onClick={() => setFilters(prev => ({ ...prev, unit: '', subunit: '', section: '', isp: '' }))}
                  ...
                  {filters.subunit && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                      ...
                      <button
                        onClick={() => setFilters(prev => ({ ...prev, subunit: '', section: '', isp: '' }))}
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    <option value="">
                      {filters.unit ? 'All Subunits' : 'Select unit first'}
                    </option>
            ...
            <div className="space-y-2">
              ...
              >
                <option value="">
                  {filters.unit ? 'All ISPs' : 'Select unit first'}
                </option>
            ...
            <div className="space-y-2">
              ...
              >
                <option value="">
                  {filters.unit ? 'All Sections' : 'Select unit first'}
                </option>
        ...
        </div>        {/* Chart and Visualization Section */}
            ...
            <div className="p-4 overflow-hidden">
                ...
                <div className="space-y-6">                  {/* Chart Container */}
                  <div className="relative bg-gray-50 rounded-lg p-2 flex justify-center w-full overflow-hidden">
                    <div className="w-full max-w-none flex justify-center">
                      <svg width={chartWidth} height={chartHeight} className="bg-white rounded-lg shadow-sm">
                      {/* Background */}
                      ...
                      {[0, 1, 2, 3, 4, 5].map(i => (
                        <line
                          key={`grid-h-${i}`}
                          x1={padding.left}
                          y1={padding.top + (i / 5) * (chartHeight - padding.top - padding.bottom)}
                          x2={chartWidth - padding.right}
                          y2={padding.top + (i / 5) * (chartHeight - padding.top - padding.bottom)}
                          stroke="#e5e7eb"
                          strokeWidth="1"
                          strokeDasharray="2,2"
                        />
                      ...
                      {[0, 1, 2, 3, 4, 5].map(i => (
                        <text
                          key={`speed-label-${i}`}
                          x={padding.left - 15}
                          y={padding.top + (i / 5) * (chartHeight - padding.top - padding.bottom) + 5}
                          textAnchor="end"
                      ...
                      {filters.unit ? (
                        ...
                        Object.entries(groupedByOffice).map(([officeName, officeData]) => {
                          const officePoints = (officeData as any[]).map(item => {
                            const dataIndex = sortedData.findIndex(d => d.date === item.date && d.office === item.office);
                      ...
                      {sortedData.map((item, index) => {
                        ...
                        return (
                          <g key={`points-${index}`}>                            {/* Download circle */}
                            {visibility.download && (
                              <circle
                                cx={x}
                                cy={downloadY}
                  ...
                  <div className="bg-gray-50 rounded-lg p-6">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-center space-x-4">
                        <button
                          onClick={() => setVisibility({ download: true, upload: true, ping: true })}
                          className="px-4 py-2 text-sm bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors shadow-sm"
                        >
                          Show All Metrics
                        </button>
                        <button
                          onClick={() => setVisibility({ download: false, upload: false, ping: false })}
                          className="px-4 py-2 text-sm bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors shadow-sm"
                        >
                          Hide All Metrics
                        </button>
                        <div className="text-sm text-gray-500 font-medium">
                          {Object.values(visibility).filter(Boolean).length} of 3 metrics visible
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-center space-x-8">
                        ...
                        >
                          <div className={`relative w-6 h-6 rounded-full transition-all duration-200 ${
                            visibility.download 
              ...
              {tooltip.visible && (
                <div
                  ...
                  style={{
                    left: Math.min(tooltip.x + 10, (typeof window !== 'undefined' ? window.innerWidth : 1200) - 340),
                ...
                >
                  ...
                  <div className="space-y-2 mb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-blue-700">{tooltip.download.toFixed(2)} Mbps</span>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-green-700">{tooltip.upload.toFixed(2)} Mbps</span>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-orange-700">{tooltip.ping.toFixed(2)} ms</span>
                    </div>
                  </div>
                  ...
                  {(tooltip.unit || tooltip.subunit || tooltip.section || tooltip.isp || tooltip.time) && (
                    <div className="pt-2 border-t border-gray-100">
                      <div className="text-xs font-semibold text-gray-700 mb-2">Location Details:</div>
                      <div className="space-y-1 text-xs">
                        ...
                        {tooltip.isp && (
                          <div className="flex items-center">
                            ...
                            <span className="ml-1 font-medium text-yellow-700">{tooltip.isp}</span>
                          </div>
                        )}
                        {tooltip.time && (
                          <div className="flex items-center">
