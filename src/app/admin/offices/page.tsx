...
export default function AdminOfficesPage() {
  ...
  const [formData, setFormData] = useState<NewOffice>({
    unitOffice: '',
    subUnitOffice: '',
    location: '',
    isp: '',
  ...
  const validateSectionISPs = (sectionISPs: { [section: string]: string[] }): boolean => {
    return Object.values(sectionISPs).every(isps => isps.every(isp => isp.trim().length > 0));
  };
  ...
  const handleDeleteOffice = async (officeId: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this office? This will remove all users, speed tests, and schedules associated with this office.'
      )
    ) {
      return;
    }
  ...
  const updateEditSectionISPs = (section: string, isps: string[]) => {
    try {
      const currentSectionISPs = editFormData.sectionISPs
        ? JSON.parse(editFormData.sectionISPs)
        : {};
      if (isps.length > 0) {
                  ...
                  <div>
                    ...
                    {selectedUnit && availableSubUnits.length === 0 && (
                      <p className="text-sm text-gray-500 mt-1">
                        No sub-units available for selected unit
                      </p>
                    )}
                  </div>
                  <div>
                  ...
                  {showAdvancedSettings && (
                    <div className="md:col-span-2 border border-gray-200 rounded-lg p-4 bg-gray-50">
                      ...
                      <p className="text-xs text-gray-600 mb-4">
                        Configure different ISPs for specific sections. Each section can have
                        multiple ISPs. If no sections are configured, the office will use the
                        general ISP settings above.
                      </p>
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center justify-between">
                                <label className="text-xs text-gray-600">
                                  ISPs for this section:
                                </label>
                                <span className="text-xs text-green-600">
                                  {
                                    (formData.sectionISPs?.[section] || ['']).filter(isp =>
                                      isp.trim()
                                    ).length
                                  }{' '}
                                  ISP(s) configured
                                ...
                                <div className="flex flex-wrap gap-1">
                                  ...
                                  {['GLOBE', 'SMART', 'PLDT', 'CONVERGE', 'SKY'].map(commonISP => (
                                    <button
                                      ...
                                      disabled={(formData.sectionISPs?.[section] || []).includes(
                                        commonISP
                                      )}
                                    >
                                      {commonISP}
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      {office.section && (
                        <div className="text-xs text-gray-500 mt-1">📋 {office.section}</div>
                      )}{' '}
                      ...
                      {(() => {
                        try {
                          ...
                          let totalSections = 0;
                          const sectionISPs = office.sectionISPs
                            ? JSON.parse(office.sectionISPs)
                            : null;
                          if (sectionISPs && Object.keys(sectionISPs).length > 0) {
                            ...
                            sectionISPCount = Object.values(sectionISPs)
                              .flat()
                              .filter(
                                (isp: any) => isp && typeof isp === 'string' && isp.trim()
                              ).length;
                          }
                          ...
                          if (totalSections > 0) {
                            return (
                              <div className="text-xs text-green-600 mt-1">
                                🔧 Advanced ISP Settings: {totalSections} section(s), {totalISPs}{' '}
                                total ISPs
                              </div>
                            );
                ...
                <div className="space-y-3">
                  {office.parentId && (
                    <div className="flex items-center text-xs text-gray-500">
                      ...
                      Sub-unit of:{' '}
                      {offices.find(o => o.id === office.parentId)?.unitOffice || 'Unknown'}
                    </div>
                  )}{' '}
                      ...
                      {(() => {
                        ...
                        try {
                          ...
                          if (office.sectionISPs) {
                            ...
                            sectionISPs =
                              typeof parsedSections === 'object' && parsedSections
                                ? parsedSections
                                : {};
                          }
                        } catch (error) {
                              ...
                              <div className="space-y-1">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-xs text-green-600">
                                    {(() => {
                                      try {
                                        const sectionISPs = editFormData.sectionISPs
                                          ? JSON.parse(editFormData.sectionISPs)
                                          : {};
                                        return (sectionISPs[section] || ['']).filter(
                                          (isp: string) => isp.trim()
                                        ).length;
                                      } catch {
                                        return 0;
                                ...
                                <div className="mt-2 pt-2 border-t border-gray-200">
                                  <div className="flex flex-wrap gap-1">
                                    <span className="text-xs text-gray-500 mr-2">Quick add:</span>
                                    {['GLOBE', 'SMART', 'PLDT', 'CONVERGE', 'SKY'].map(
                                      commonISP => (
                                        <button
                                          key={commonISP}
                                          type="button"
                                          onClick={e => {
                                            ...
                                            try {
                                              ...
                                              // Check if ISP already exists
                                              if (!currentISPs.includes(commonISP)) {
                                                const newISPs = currentISPs.filter((isp: string) =>
                                                  isp.trim()
                                          ...
                                          disabled={(() => {
                                            try {
                                              const sectionISPs = editFormData.sectionISPs
                                                ? JSON.parse(editFormData.sectionISPs)
                                                : {};
                                              return (sectionISPs[section] || []).includes(
                                                commonISP
                                              );
                                            } catch {
                                              return false;
