...
export async function GET(request: NextRequest) {
  ...
  const cleanup = () => {
    ...
    console.log(`🧹 [${requestId}] Request cleaned up. Remaining active requests: ${activeRequests.size}`);
  };
  ...
  if (session.user?.role !== 'ADMIN' && session.user?.officeId !== officeId) {
    console.log(`🚫 [${requestId}] Forbidden - user office ${session.user?.officeId} doesn't match requested ${officeId}`);
  ...
  const stream = new ReadableStream({
    start(controller) {      const safeEnqueue = (data: string) => {
      ...
      speedtest.stdout.on('data', (data) => {
        ...
        console.log(`📊 [${requestId}] Progress ${progressCount} at ${elapsed}ms - received ${chunk.length} bytes`);
          ...
          if (trimmedLine.startsWith('{')) {
            try {
              ...
              if (result.type === 'testStart') {
                ...
                console.log(`🏓 [${requestId}] Ping phase: ${(pingProgressPercent * 100).toFixed(1)}% (${overallProgress.toFixed(1)}% overall) - ${currentPing.toFixed(1)}ms`);
                ...
                const downloadProgressPercent = Math.max(0, Math.min(1, result.download.progress || 0)); // Ensure 0-1 range
                ...
                console.log(`⬇️ [${requestId}] Download phase: ${(downloadProgressPercent * 100).toFixed(1)}% (${overallProgress.toFixed(1)}% overall) - ${currentDownload.toFixed(1)} Mbps`);
                ...
                console.log(`⬆️ [${requestId}] Upload phase: ${(uploadProgressPercent * 100).toFixed(1)}% (${overallProgress.toFixed(1)}% overall) - ${currentUpload.toFixed(1)} Mbps`);
                ...
                console.log(`🎯 [${requestId}] Final results after ${testDuration}ms: Download ${finalDownload.toFixed(1)}Mbps, Upload ${finalUpload.toFixed(1)}Mbps, Ping ${finalPing.toFixed(1)}ms`);
                ...
                const detectedISP = result.isp || result.interface?.externalIsp || result.client?.isp || 'Unknown ISP';
                ...
                console.log(`📤 [${requestId}] Sending final results to client`);                safeEnqueue(`data: ${JSON.stringify({
                  type: 'result',
                  stage: 'complete',
                  progress: 100,
                  download: finalDownload,
        ...
        if (errorText.includes('No servers') || errorText.includes('Unable to connect')) {
          ...
          safeEnqueue(`data: ${JSON.stringify({
            type: 'error',
            error: 'Unable to connect to speedtest servers. Please check your internet connection.'
          })}\n\n`);
          speedtest.kill();        } else if (errorText.includes('Cannot open socket') || errorText.includes('socket')) {
          console.log(`🔌 [${requestId}] Socket error detected`);
          hasError = true;
          safeEnqueue(`data: ${JSON.stringify({
            type: 'error',
            error: 'Network socket error. This could be due to firewall restrictions or network configuration. Please try again or contact your network administrator.',
            networkError: true
          })}\n\n`);
          speedtest.kill();        } else if (errorText.includes('Unknown error') || errorText.includes('Error: [0]')) {
          console.log(`❓ [${requestId}] Unknown error detected`);
          hasError = true;
          safeEnqueue(`data: ${JSON.stringify({
            type: 'error',
            error: 'Speedtest CLI encountered an unknown error. This may be temporary - please try again in a few moments.',
            retryable: true
          })}\n\n`);
          speedtest.kill();
        } else if (errorText.includes('Protocol error') || errorText.includes('Did not receive HELLO')) {
            ...
            if (jsonMatch) {
              ...
              setTimeout(async () => {
                try {
                  ...
                  console.log(`💾 [${requestId}] Speed test saved to database (fallback): ${speedTest.id}`);
                } catch (dbError) {
                  console.error(`❌ [${requestId}] Failed to save speed test to database (fallback):`, dbError);
                }
              }, 100);
            }
          } catch (error) {
